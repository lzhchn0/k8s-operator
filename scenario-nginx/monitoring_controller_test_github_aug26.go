...
func TestListService(t *testing.T) {
	...
	s2, err := getService("default", "my-service-02", 9092)
	fmt.Println("==099-2 dump Service  ")
	scs3.Dump(s1)

	_ = err
	var resources []runtime.Object
	resources = append(resources, s1)
	resources = append(resources, s2)
	fmt.Println("==099-3 dump Service # : ", len(resources))

	fakeClient := fake.NewClientBuilder().
		WithScheme(appsScheme).
	...
	fmt.Println("==099-4 dump Service  ")
	scs3.Dump(svcs)
